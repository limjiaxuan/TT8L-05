...
class TaskApp:
    ...
    def create_widgets(self):
        ...
        # Add Pomodoro button
        btn_pomodoro = tk.Button(sidebar_frame, text="Pomodoro Timer", width=20, highlightbackground="lightgrey", command=self.show_pomodoro_popup)
        ...
        self.frames = {
            ...
            "pomodoro": tk.Frame(self.main_frame)
        }
    ...
    def save_task_to_db(self, task):
        self.cursor.execute("INSERT INTO tasks (name, description, due_date, user_id) VALUES (?, ?, ?, ?)",
                            (task["name"], task["desc"], task["due_date"], self.current_user["id"]))
        self.conn.commit()
    ...
    def show_pomodoro_popup(self):
        ...
        pomodoro_window = Toplevel(self.master)
        pomodoro_window.title("Pomodoro Timer")

        tk.Label(pomodoro_window, text="Select Task for Pomodoro", font=("Helvetica", 14)).pack(pady=10)
        
        task_listbox = tk.Listbox(pomodoro_window, selectmode=tk.SINGLE)
        task_listbox.pack(fill=tk.BOTH, padx=20, pady=10, expand=True)
        
        for task in self.tasks:
            task_listbox.insert(tk.END, f"{task['name']} - {task['desc']}")

        timer_frame = tk.Frame(pomodoro_window)
        timer_frame.pack(pady=20)

        self.timer_label = tk.Label(timer_frame, text="25:00", font=("Helvetica", 24))
        self.timer_label.pack(pady=5)

        button_frame = tk.Frame(pomodoro_window)
        button_frame.pack(pady=10)

        start_button = tk.Button(button_frame, text="Start", command=lambda: self.start_pomodoro(task_listbox))
        start_button.pack(side=tk.LEFT, padx=5)

        reset_button = tk.Button(button_frame, text="Reset", command=self.reset_pomodoro)
        reset_button.pack(side=tk.LEFT, padx=5)

        self.pomodoro_time = 25 * 60
        self.pomodoro_running = False

    def start_pomodoro(self, task_listbox):
        if self.pomodoro_running:
            return
        selected_task_idx = task_listbox.curselection()
        if not selected_task_idx:
            messagebox.showerror("Error", "Please select a task to start the Pomodoro timer.")
            return
        self.selected_task = self.tasks[selected_task_idx[0]]
        ...
        if self.pomodoro_running:
            if self.pomodoro_time > 0:
                ...
                notification.notify(
                    ...
                    message=f"Pomodoro session completed for task: {self.selected_task['name']}",
                    timeout=10
                )
                self.show_reminder_popup(self.selected_task)
                self.reset_pomodoro()

...
class TaskApp:
    ...
    def load_tasks_from_db(self):
        ...
        for row in rows:
            print(f"Row: {row}")  # Debugging print to check the row
            if len(row) == 5:
                task = {"id": row[0], "name": row[1], "desc": row[2], "due_date": row[3], "icon_path": row[4]}
                self.tasks.append(task)
                self.display_task(task)
            else:
                print("Row length mismatch. Expected 5 columns, got:", len(row))


    def save_task_to_db(self, task):
        self.cursor.execute("INSERT INTO tasks (name, description, due_date, icon_path) VALUES (?, ?, ?, ?)",
                            (task["name"], task["desc"], task["due_date"], task["icon_path"]))
        self.conn.commit()
    ...
    def show_add_task_popup(self):
        ...
        def select_icon():
            ...
            for icon_path in icon_paths:
                ...
                resized_icon = icon_image.subsample(3)
    ...
    def display_task(self, task, frame=None):
        ...
        if task["icon_path"] and task["icon_path"] != "Not selected":
            ...
            icon_label = tk.Label(task_frame, image=icon_image)
